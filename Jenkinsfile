    ...
    try {
      ...
      if (utils.isCI()) {
        ...
        stage('notify'){
            ...
            container('clients'){
                flow.addCommentToPullRequest(message, pr, 'fabric8-services/fabric8-tenant-team')
            }
        }

      } else if (utils.isCD()) {
        sh "git remote set-url origin git@github.com:fabric8-services/fabric8-tenant-team.git"
